...
RUN apk add bash supervisor curl git openssl nginx php8-fpm php8-json php8-cli php8-sockets && \
	apk add php8-openssl php8-fileinfo php8-mbstring php8-gd php8-mysqli php8-pecl-mongodb php8-phar && \
    ...
    cd /root && curl -4 -s https://getcomposer.org/installer | php && \
	chown -R www-data:www-data /app && chmod -R 770 /app && \
    cd /app && php /root/composer.phar install
